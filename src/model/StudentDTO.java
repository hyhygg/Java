...
// M: Model
//    모델은 컨트롤러가 뷰어에 데이터를 보내줄때나, 뷰어가 보내준 데이터를 데이터베이스로 보낼 때
...
//    만약 해당 객체의 값이 수정도 되는 경우에는 DTO, 값을 읽어오는 용도일 때에는 VO 라고 하지만
//    사실 의미하는 바가 큰 차이가 없으므로 둘 중 아무거나 적어도 된다.
// V: Viewer
//    뷰어는 화면에 출력을 담당하는 클래스가 된다.
//    다만 웹을 통해서 출력을 할 때에는 뷰어 클래스 없을 수도 있는데
//    컨트롤러가 직접 웹에 필요한 데이터를 전송하게 되면, 뷰어 클래스는 생략 가능하기 때문.
//    만약 웹인데도 뷰어 클래스를 사용하는 경우에는, 웹에서 출력할 내용을 뷰어가 모두다 만들어서 보내주는 경우가 그런 경우이다.
//    뷰어의 경우 이름이 Viewer로 끝난다.
// C: Controller
//    컨트롤러는 뷰어의 요청에 따라서 데이터를 가공하거나 보내주는 역할을 맡는 클래스가 된다.
//    데이터베이스가 있을 경우에는 뷰어의 요청에 맞춰 CRUD를 하게 된다.
//    컨트롤러의 경우 이름이 Controller로 끝난다.

// CRUD란?
// CRUD는 데이터베이스의 가장 기본적 기능인데
