    ...
    private static void showMenu() {
        while (true) {
            ...
            if (userChoice == 1) {
                ...
                // 지금은 콘솔이기 때문에 별 차이가 없는 것처럼 보이지만
                // 나중에 웹 프로젝트에서는
    ...
    private static void printAllBoard() {
        ...
        while (true) {
            if (boardList.isEmpty()) {
                ...
                // 게시글의 작성일을 형식에 맞춰줄 SimpleDateFormat 객체 저장
...
//                            sdf.format(boardList.get(i).getWrittenDate()));
                    System.out.printf("%d번. %s \t\t%s\n", b.getId(), b.getTitle(),
    ...
    private static void printOneBoard(int id) {
        // 게시글의 상세보기와 헤당 게시글의 댓글을 출력해주는 메소드

        // 파라미터로 받은 id 값을 사용하여
        // 게시글 정보를 불러온다.
        Board b = retrieveBoard(id);

        // b가 null 일 경우, id가 잘못되었다는 의미이므로
        // 경고 메시지만 출력
        ...
        if (b == null) {
            ...
            if (list.isEmpty()) {
                System.out.println("         등록한 댓글이 존재하지 않습니다.        ");
                ...
                if (userChoice == 1) {
                    ...
                    if (list.isEmpty()) {
                        ...
                        if (userInput != 0) {
                            ...
                            // 현재 댓글번호 밖에 저장이 안된 Reply 객체 r에 나머지 정보를 넣어준다.
    ...
    private static void deleteBoard(int boardId) {
        // 먼저 정말로 게시글을 삭제할지 물어본다.
        String message = new String("게시글을 정말로 삭제하시겠습니까? 삭제하시려면 Y 혹은 y를 입력해주세요. ");
    ...
    // 회원번호로 해당 회원의 닉네임을 찾아서 리턴하는 selectNameById() 메소드
...
//          if (boardId == replyList.get(i).getBoardId()) {
