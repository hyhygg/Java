...
// 1. 접근제한자
//    접근제한자란, 해당 메소드가 어디에 있는 다른 클래스가 사용 가능한지 규정하는 키워드이다.
...
//    package:   패키지 접근 제한자는 오직 같은 패키지에 있는 다른 클래스들만 해당 메소드를 사용할 수 있다.
...
//            리턴 타입이 존재할 때에는 해당 메소드 내부에 반드시 return 이라는 키워드를 사용해서, 그 데이터 타입과 일치하는 데이터 타입을 가진
//            값을 반드시 return 해주어야 한다.
...
// 5. 파라미터: 파라미터란, 해당 메소드를 실행 시킬때 외부로부터 꼭 받아와야 하는 특별한 종류의 데이터가 있을 경우
//           우리가 파라미터 안에 적어서 "이 메소드를 실행 시킬 때에는 이게 필요합니다!" 라고 적어주는 것이다.
//           파라미터는 우리가 한국말로 "매개 변수" 라고도 하는데, 왜냐면 메소드 전체에서 해당 파라미터는 메소드 전체가 유효범위인 변수가 된다.
//           파라미터는 우리가 변수를 선언하듯, 데이터타입 이름 요렇게 적어주면 된다.
//           또한 우리가 파라미터에 어떠한 이름을 적더라도, 실제 그 메소드를 호출할 때에는 그 이름의 변수를 꼭 넣어줄 필요는 없지만
...
public class Ex02Method {
    ...
    // 사용자한테 1~10 사이의 랜덤한 숫자를 리턴해주는 메소드
    public static int selectRandom() {
        Random random = new Random();
        int max = 10;

        // 우리가 굳이 int 타입의 변수만 return 해줄수 있는 것이 아니라
