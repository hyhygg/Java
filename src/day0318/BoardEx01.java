...
public class BoardEx01 {
    ...
    private static void selectAll() {
        while (true) {
            ...
            // 사용자로부터 개별 보기할 글의 id나 혹은 뒤로가기를 선택하도록 입력을 받는다.
            String message = new String("개별 보기할 글의 번호를 입력해주세요. 만약 메뉴로 가시려면 0을 입력해주세요.");
    ...
    private static void selectOne(int id) {
        ...
        // Board 객체 b에 파타미터로 넘어온 id 값을 넣어준다.
        b.setId(id);
    ...
    private static void delete(int id) {
        ...
        if (index == -1) {
            ...
            if (yesNo.equalsIgnoreCase("Y")) {
                // 3. 사용자가 대소문자 y를 입력했을 경우, 해당 index를 삭제한다.
                // boardList의 remove(index) 실행
                boardList.remove(index);
            } else {
                // 4. 그 외에 다른 글자를 입력했을 경우, selectOne(Index)를 실행한다.
                selectOne(id);
            }
        }
    }
