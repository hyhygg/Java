...
public class StudentViewer {
    ...
    public void register() {
        ...
        // 1-8. 정보가 들어가 있는 학생객체를 컨트롤러에 add() 한다.
        controller.add(s);
    }
    ...
    // 3. 학생 사용자가 로그아웃할 때,
    // 호출돼서 logIn 객체를 null로 만들어주는 메소드
    public void logOut() {
    ...
    // 만약 파라미터로 받은 로그인한 학생의 객체의 값이 null이면 교사가 로그인해서 실행한 걸로 생각하면 된다.
    // int type의 경우 VALUE_ALL 이면 모든 학생 보여주기 이고,
    // VALUE_NO_INPUT 이면 아직 성적입력이 안된 학생으로 생각하면 된다.
    public void printList(int group, int type) {
        ...
        while (true) {
            ...
            String message = new String("개별 보기할 번호를 입력하시거나 뒤로 가시려면 0을 입력해주세요.");
            int id = ScannerUtil.nextInt(scanner, message);

            // 3-3. 입력한 번호로 controller의 selecOne()을 실행한다.
            StudentDTO s = controller.selectOne(id);

            // 3-4. 번호가 유효한 번호인지 확인한다.
    ...
    public void printOne(int id) {
        ...
        // 1. 입력/수정 2. 뒤로가기 를 보여주고
        // 그 외에는
        // 1. 뒤로가기를 보여준다.

        // logIn 값에 따라서 같이 값이 달라지는 String message
        String message;
        // 로그인이 null 이면 1. 뒤로가기, 그 외에는 1. 입력/수정 2. 뒤로가기 가 message의 값으로 정해진다.
        if (logIn == null) { // 학생X, 교사로써 로그인
        ...
        // 1~2까지 입력을 받고, 그 외에는 1만 입력을 받게 된다.
    ...
    // 현재 로그인 객체가 null 이면 true, null 이 아니면 false가 리턴되는 메소드
    public boolean isLogInNull() {
        return logIn == null;   
    }
    
    // 현재 로그인 객체의 group 을 리턴해주는 메소드
