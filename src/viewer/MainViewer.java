...
// 여러가지 뷰어를 모아서
// 특정 사항마다 해당 뷰어에 필요한 메소드를 실행해주고
// 메뉴 출력 등을 담당하는 메인뷰어 클래스
...
public class MainViewer {
            ...
            if (userChoice == 1) {
                ...
                if (userChoice == 1) {
                    ...
                    if (studentViewer.logIn()) {
                        ...
                        showStudentMenu();
                        // 메인 화면 메소드 종료 후에는
                        // studentViewer.logOust() 실행해서 로그인 객체 초기화
    ...
    private void showStudentMenu() {
        while (true) {
            ...
            if (userChoice == 1) {
                ...
                // 지금은 studentViewer의 StudentDTO 객체 logIn이 null 이 아니므로
                // 아무값이나 파라미터로 넘겨주면 된다.
    ...
    private void showTeacherMenu(int group) {
        ...
        while (true) {
            ...
            String message = new String("1. 반 전체 학생 보기 2. 반 학생 중 성적 미입력 학생보기 3. 자기 평가보기 4. 종료");
            int userChoice = ScannerUtil.nextInt(scanner, message, 1, 4);

            if (userChoice == 1) {
                // StudentViewer의 printAll을 실행하되,
                // printList의 경우, 파라미터로 group과 출력할 내용을 선택해주어야 하는데
                // 지금은 전체보기 이므로 private final in VALUE_ALL = 1;
                // 이 출력할 내용의 옵션 값이다.
                // group은 파라미터로 넘어온 group을 사용하면 된다.
                studentViewer.printList(group, VALUE_ALL);
            } else if (userChoice == 2) {
                // studentViewer의 printList의
                // group, VALUE_NO_INPUT 으로 실행해준다.
