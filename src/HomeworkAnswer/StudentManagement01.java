...
public class StudentManagement01 {
    ...
    // 학생 로그인시 로그인한 학생의 정보를 저장할 Student 객체
    private static Student LogInStudent = null;
    // 교사 로그인시 로그인한 교사의 정보를 저장할 Teacher 객체
    ...
    private static void register() {
        ...
        // 2. 로그인 시 사용할 아이디, 로그인 시 사용할 비밀번호, 이름을 받는다.
    ...
    private static void printAll(ArrayList<Student> list) {
        while (true) {
            // list 가 널이란 것은
            // 우리가 성적 입력 안된 학생 리스트를 보겠다고 선택한 상황이기 때문에
            // 리스트에 selectByInputSwitch()를 실행한 결과값을 할당해주면 된다.            
            if(list == null) {
                list = selectByInputSwitch();
            }
            // 지금 현재 소속반의 리스트를 for문을 이용하여 전부 출력한 후에
            // 개별 보기, 뒤로 가기 를 한다.
            // 단, for문으로 출력시 성적이 입력되었는지를 따로 표시해준다.
            ...
            String message = new String("개별 보기할 번호를 입력해주세요. 만약 뒤로 가시려면 0을 입력해주세요.");
            int userChoice = ScannerUtil.nextInt(scanner, message);

            // 2. 사용자가 입력한 개별 보기 번호가 유효한지 확인한다.
            // 만약 유효하지 않으면 유효한 값을 입력할 때까지 다시 입력을 받는다.

            // 2-1. 입력한 값이 유효한지 확인하기 위해 id가 userChoice인 Student 객체를 만든다.
            Student s = new Student();
            s.setId(userChoice);
            ...
            // 4. 파라미터 list와 전역변수 groupStudents 를 비교하여
            //    둘이 같지 않을 경우, list에 selectByInputSwitch()를 다시 실행해준다.
            //    그렇게 할 경우, 만약 우리가 성적 입력 안된 학생 리스트를 뽑아왔었는데
    ...
    private static void printOne(int id) {
        ...
        if (LogInStudent != null) {
            ...
            // 또한, inputSwitch도 입력 수정 모두 inputSwitch 값을 true로 바꿔버리면 해결이 더 쉽다.
    ...
    private static void insertScore(int id) {
        ...
        // 만약 이미 기존에 성적을 입력한 학생이였으면
        // scoreInputSwitch 필드의 값이 true였는데 true로 바뀌게 된다.
        // 즉 변화가 없는 것이다.
        
        // 만약 기존에 성적을 입력하지 않은 학생이였으면
        // scoreInputSwitch 필드의 값이 false였는데 true로 바뀌게 된다.
        // 즉 해당 필드의 역할을 다 하게 되는 것이다.
        s.setScoreInputSwitch(true);

        // printOne 메소드를 다시 호출해서 입력/수정된 점수를 볼 수 있게 한다.
        printOne(id);
    }
    ...
    private static void showNotGraded() {
        ...
        // 무한루프 안에서 성적 입력이 안된 학생들의 목록을 봅아준다.
        while(true) {
             for(Student s : list) {
