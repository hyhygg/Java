...
public class ArrayUtil {
    ...
    // 2. 파라미터로 배열과 int 값을 넘겨 받고, 해당 int 값이 배열에 존재하면 true,
    ...
    // 3. 파라미터로 배열과 int 값을 넘겨 받고, 해당 int 값이 가장 먼저 등장하는 index를 알려주는
    // indexOf() 메소드,
    ...
    // 해당 인덱스를 제거하고
    // 크기도 1칸 줄어든 배열을 리턴하는 remobeByIndex() 메소드
    public static int[] removeByIndex(int[] arr, int index) {
        // 1. 만약 index가 유효하지 않은 index 일 경우,
        // arr을 그대로 리턴한다.
        if (!(index >= 0 && index < size(arr))) {
            return arr;
        }
